...
public class SmugglingFilter implements Filter
{
    ...
    public void doFilter(ServletRequest req, ServletResponse res, FilterChain chain) throws IOException, ServletException
    {
        ...
        List<String> encodings = encodingEnumeration == null ? new ArrayList<>() : Collections.list(encodingEnumeration);
        // TE-TE
        if (encodings.size() > 1) {
        ...
        List<String> lengths = lengthEnumeration == null ? new ArrayList<>() : Collections.list(lengthEnumeration);
        if (lengths.size() > 1) {
